...
<div class="container">
        ...
        <form method="GET" action="{{ request()->url() }}">
            <div class="filter-row">
                <div class="filter-group">
                    <label for="search">Cari Nama Pupuk</label>
                    <input type="text" 
                           name="search" 
                           id="search" 
    ...
    {{-- Info total data --}}
    @if(isset($stokMasukHistory) && $stokMasukHistory->total() > 0)
        <div class="alert alert-info">
            <i class="fas fa-info-circle"></i> 
            Total data: <strong>{{ $stokMasukHistory->total() }}</strong> stok masuk
        </div>
    @endif
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                @forelse ($stokMasukHistory as $item)
                    <tr>
                        <td>
                            @if(method_exists($stokMasukHistory, 'firstItem') && $stokMasukHistory->firstItem())
                                {{ ($stokMasukHistory->currentPage() - 1) * $stokMasukHistory->perPage() + $loop->iteration }}
                            @else
                                {{ $loop->iteration }}
                            @endif
                        </td>
    ...
    {{-- Pagination --}}
    @if(isset($stokMasukHistory) && $stokMasukHistory->hasPages())
        <div class="pagination-wrapper">
            <div class="pagination-info">
                Menampilkan {{ $stokMasukHistory->firstItem() }} - {{ $stokMasukHistory->lastItem() }} dari {{ $stokMasukHistory->total() }} data
            </div>
            
            <nav>
                <ul class="pagination">
                    {{-- Previous Page Link --}}
                    @if ($stokMasukHistory->onFirstPage())
                        <li class="disabled"><span>‹ Sebelumnya</span></li>
                    @else
                        <li><a href="{{ $stokMasukHistory->appends(request()->query())->previousPageUrl() }}" rel="prev">‹ Sebelumnya</a></li>
                    @endif

                    {{-- Pagination Elements --}}
                    @foreach ($stokMasukHistory->getUrlRange(1, $stokMasukHistory->lastPage()) as $page => $url)
                        @if ($page == $stokMasukHistory->currentPage())
                            <li class="active"><span>{{ $page }}</span></li>
                        @else
                            <li><a href="{{ $stokMasukHistory->appends(request()->query())->url($page) }}">{{ $page }}</a></li>
                        @endif
                    @endforeach

                    {{-- Next Page Link --}}
                    @if ($stokMasukHistory->hasMorePages())
                        <li><a href="{{ $stokMasukHistory->appends(request()->query())->nextPageUrl() }}" rel="next">Selanjutnya ›</a></li>
                    @else
